...
import { Settings, Hash, Edit3, Copy, ExternalLink, X } from "lucide-react"
import type { Dispatch, SetStateAction } from "react"
import { useState } from "react"

interface SubDepartmentDetailPopupProps {
  open: boolean
...
export default function SubDepartmentDetailPopup({ open, setOpen, subDepartmentData }: SubDepartmentDetailPopupProps) {
  const data = {
    ...
    >
      <div className="space-y-8">
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-gray-50/50 to-slate-50/50 rounded-2xl"></div>
          <div className="relative flex items-start gap-4 p-6 rounded-2xl border border-gray-100/50 backdrop-blur-sm">
            <div className="relative">
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-gray-500 to-slate-600 shadow-lg shadow-gray-500/25">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-md font-semibold text-gray-900 truncate mb-1">{data.subDepartmentName}</h1>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center px-3 py-1 text-xs font-semibold bg-gradient-to-r from-gray-50 to-slate-50 text-gray-700 rounded-full border border-gray-200/50 shadow-sm">
                  {data.subDepartmentCode}
                </span>
        ...
        {/* Sub Department Information with enhanced cards */}
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-800 tracking-wide uppercase">Sub Department Information</h2>
