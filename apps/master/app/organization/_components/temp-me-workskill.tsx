...
export default function WorkSkillManagement() {
  ...
  const totalPages = Math.ceil(filteredWorkSkills.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const paginatedWorkSkills = filteredWorkSkills.slice(startIndex, startIndex + itemsPerPage)
          ...
          <CardContent className="p-0">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t">
              <div className="text-sm text-gray-600">
                Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredWorkSkills.length)} of{" "}
                {filteredWorkSkills.length} entries
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
